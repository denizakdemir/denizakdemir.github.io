...
- **IWM-EFA (both equities):** `r iwm_efa_sim`
- **AGG-TLT (both bonds):** `r agg_tlt_sim`  
- **IWM-GLD (equity vs gold):** `r iwm_gld_sim`
- **GLD-DBC (both commodities):** `r gld_dbc_sim`

These similarities make intuitive sense: bonds are highly similar to each other, commodities cluster together, while equity and gold show little similarity.
...
The prepared dataset contains `r n_observations` observations across `r n_assets` assets spanning `r n_periods` time periods (months).
